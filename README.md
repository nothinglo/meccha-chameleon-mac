...
3. **（重要，設定日後啟動）** 回 **Configure 視窗** → 把 **Windows app（主程式）** 設成 `Steam.exe`。
   設好後，以後只要點兩下 `~/Applications/Sikarugir/` 裡的 wrapper `.app`，就會自動開 Steam。
...
3. **(Important — sets up future launching)** Back in the **Configure window**, set **Windows app** to `Steam.exe`.
   After this, double-clicking the wrapper `.app` in `~/Applications/Sikarugir/` will open Steam directly.
