...
- 較新的 macOS
- 數 GB 可用空間（每款遊戲另計）
- ⏱ 整個處理過程約 **30 分鐘**（環境安裝 + 建 wrapper + 裝 Steam）
...
3. **（重要，設定日後啟動）** 回 **Configure 視窗** → 在 **Windows app（主程式）** 的下拉選單選 `Steam.exe`。
   - 下拉選單通常**已經有 `Steam.exe`**（剛裝好的）；若沒看到，按 **Browse** 到 `C:\Program Files (x86)\Steam\Steam.exe`（即 wrapper 內 `Contents/SharedSupport/prefix/drive_c/Program Files (x86)/Steam/Steam.exe`）。
...
- ⏱ The whole process takes about **30 minutes** (setup + create wrapper + install Steam)
...
3. **(Important — sets up future launching)** Back in the **Configure window**, pick `Steam.exe` from the **Windows app** dropdown.
   - The dropdown usually **already lists `Steam.exe`** (you just installed it); if not, click **Browse** to `C:\Program Files (x86)\Steam\Steam.exe` (i.e. `Contents/SharedSupport/prefix/drive_c/Program Files (x86)/Steam/Steam.exe` inside the wrapper).
