...
echo "3. （重要）先補中文字型！這樣 Steam 安裝畫面與遊戲內中文才不會變方框。回終端機跑："
...
echo "4. Configure 視窗按 Install Software → 選【~/Downloads/SteamSetup.exe】"
echo "   安裝視窗出現後就能操作，一路按「下一步/確定」用預設值即可(此時 Configure 視窗可能顯示 busy，正常)"
echo "   安裝過程若跳警告(warning)是正常的，不用擔心，繼續即可"
echo "   ⚠️ 若 Steam 安裝視窗沒跳出來，點【右下角/Dock 的『藍底 wine 圖示』】(不是酒杯，酒杯是 Configure)把它叫出來"
echo "   裝完過幾分鐘 Steam 會自動打開"
echo "5. Steam 打開後 → 登入 → 安裝你的遊戲（例如 MECCHA CHAMELEON）"
